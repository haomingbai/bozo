 ...
 *
 * This class decoupling the `Connection` object from the `bozo::cancel()` operation to
 ...
 *
 * @param handle --- handle for the cancel operation, should be obtained via `bozo::get_cancel_handle`.
 ...
 * * Callback with signature `void(bozo::error_code ec, std::string error_message)`. In this case `bozo::cancel()` returns `void`.
 ...
 *   to get a future on the asynchronous operation result. In this case `bozo::cancel()` will return `std::future<void>`. Additional
...
boost::asio::spawn(io, [&io, &timer](auto yield){
    ...
    boost::asio::spawn(yield, [&io, &timer, handle = get_cancel_handle(conn)](auto yield) mutable {
        ...
        bozo::error_code ec;
        auto error_msg = bozo::cancel(std::move(handle), io, 5s, yield[ec]);
        if (ec) {
            std::cerr << ec.message() << ", " << error_msg << std::endl;
        }
    });
    bozo::execute(conn, "SELECT pg_sleep(1000000)"_SQL, yield[ec]);
    assert(ec == bozo::sqlstate::query_canceled);
});
 ...
 *
 * @param handle --- handle for the cancel operation, should be obtained via `bozo::get_cancel_handle`.
 * @param token --- valid `CancelCompletionToken` (see time constraint version of `bozo::cancel()` for details).
 * @return deduced from `CancelCompletionToken`.
 *
