...
#include <bozo/core/recursive.h>
#include <bozo/core/none.h>
#include <bozo/deadline.h>
#include <bozo/pg/handle.h>
...
using no_statistics = bozo::none_t;
 ...
 * it is better to specialize `bozo::unwrap_connection_impl` template for custom type.
 * E.g. such overload is used for the `bozo::pooled_connection` of this library.
...
class connection {
public:
    using native_handle_type = bozo::pg::conn::pointer; //!< Native connection handle type
     ...
     */
    error_code assign(bozo::pg::conn&& handle);
     ...
     *
     * @return bozo::pg::conn --- native connection handle object
     */
    bozo::pg::conn release();
     ...
     *
     * @param handler --- wait handler with `void(bozo::error_code, int=0)` signature.
     */
...
* @par Concrete models
*
* `bozo::connection`, `bozo::pooled_connection`, `bozo::transaction`
*
* @par Definition
*
* Any wrapper object, which may be unwrapped to the underlying `%Connection` model via
* `bozo::unwrap_connection` is valid `%Connection` model.
*
...
* | <PRE>bozo::get_connection(c, t, Handler)</PRE> | | Should reset the additional error context. This behaviour is performed in default implementation of `bozo::get_connection()` via `%c.set_error_context()` call. |
* | <PRE>bozo::is_connection<C></PRE> | `std::true_type` | The template `bozo::is_connection` should be specialized for the connection type via inheritance from `std::true_type`. |
*
* Where:
* * `Handler` is callable with `template <typename Connection> void(bozo::error_code, Connection&&)` signature,
* * `WaitHandler` is callable with `void(bozo::error_code, int = 0)` signature,
* * `t` is a `TimeConstraint` model object,
* * `as_const()` is `std::as_const()`.
*
* @sa bozo::get_connection(), bozo::is_connection
* @concept{Connection}
*/
//! @cond
template <typename T>
 ...
 *
 * Alias to `!bozo::connection_bad(conn)`.
 *
 * @param conn --- `Connection` object.
...
template <typename T>
struct get_connection_type_default<T, Require<bozo::Connection<T>>> {
    using type = T;
};
 ...
 * `ConnectionSource` or `ConnectionProvider`. It uses `bozo::get_connection_type` metafunction
 ...
 * `bozo::connection_provider` or other `%ConnectionSource` models like `bozo::connection_pool`.
 * Typically there is no need to use it directly.
 *
 * @par Concrete models
 *
 * `bozo::connection_info`, `bozo::connection_pool`, `bozo::failover::role_based_connection_source`.
 *
 * @par Definition
 *
 ...
 * | Expression | Type | Description |
 * |------------|------|-------------|
 * | <PRE>s(io, t, h)</PRE> | | Should invoke a continuation object `h` within time-limited by `t` (if not `bozo::none` specified) with a connection object associated with an executor object of the `io` execution context or/and `error_code` in case of failure. If there is no error_code code set, the connection object should be in a valid state. |
 * | <PRE>bozo::connection_type<S></PRE> | deduced type | Should return the `Connection` object type that is produced by the source. |
 * | <PRE>bozo::Connection<bozo::connection_type<S>></PRE> | `std::true_type` | Should provide a connection type that models the `Connection` concept. |
 *
 * Where:
 * * `io` execution context object with type `bozo::io_context`,
 * * `t` is the `TimeConstraint` model object,
 * * `h` is callable with `void(bozo::error_code, bozo::connection_type<S>)` signature.
 *
 ...
 * object.
 *
 * Typically it is enough to use the `ConnectionSource` model with `bozo::connection_provider`,
 ...
 * @par Concrete models
 *
 * `bozo::connection_provider`, `Connection` model is a `%ConnectionProvider` by default.
 *
 * @par Definition
 *
 ...
 * | Expression | Type | Description |
 * |------------|------|-------------|
 * | <PRE>bozo::get_connection(p, t, h)</PRE> | | Should invoke a continuation object `h` within time-limited by `t` (if not `bozo::none` specified) with a connection object associated with an executor object or/and `error_code` in case of failure. If no `error_code` code is set the connection object should be in a valid state. |
 * | <PRE>bozo::connection_type<P></PRE> | deduced type | Should return a type of the `Connection` object that is produced by the provider. |
 * | <PRE>bozo::Connection<bozo::connection_type<P>></PRE> | `std::true_type` | Should provide a connection type that models the `Connection` concept. |
 *
 * Where:
 * * `t` is the `TimeConstraint` model object,
 * * `h` is callable with `void(bozo::error_code, bozo::connection_type<S>)` signature.
 ...
 *
 * This function is a shortcut to `bozo::get_connection(provider, bozo::none, token)` call.
 *
 * @note The function does not particitate in ADL since could be implemented via functional object.
 *
 * @param provider --- `ConnectionProvider` to get connection from.
 ...
 *
 * This function creates RAII guard object which calls `bozo::close_connection()`
 * at the end of its scope. If nullptr is passed as argument or connection is
 * null recursive no `bozo::close_connection()` would be made.
 *
...
inline auto defer_close_connection(Connection* conn) {
    static_assert(bozo::Connection<Connection>, "argument should model Connection");
...
#include <bozo/impl/connection.h>
