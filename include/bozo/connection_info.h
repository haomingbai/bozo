...
public:
    using connection_type = std::shared_ptr<bozo::connection<OidMap, Statistics>>; //!< Type of connection which is produced by the source.
    ...
    void operator ()(io_context& io, TimeConstraint t, Handler&& handler) const {
        ...
        impl::async_connect(conn_str, t, std::allocate_shared<bozo::connection<OidMap, Statistics>>(allocator, io, statistics),
...
/**
 * @brief Constructs `bozo::connection_info` `ConnectionSource`.
 * @ingroup group-connection-functions
 * @relates bozo::connection_info
 *
 ...
 * @return `bozo::connection_info` specialization.
 */
