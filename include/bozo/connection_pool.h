...
#include <bozo/asio.h>
#include <bozo/connector.h>
#include <bozo/core/thread_safety.h>
#include <bozo/detail/connection_pool.h>
 ...
 *
 * Configuration of the `bozo::connection_pool`, e.g. how many connection are in the pool,
...
/**
 * @brief [[DEPRECATED]] Timeouts for the bozo::get_connection() operation
 * @ingroup group-connection-types
 *
 * Time restrictions to get connection from the `bozo::connection_pool`
 * @note This type is deprecated, use time constrained version of bozo::get_connection() or bozo::dedline for operations.
 */
...
public:
    ...
    using native_handle_type = typename bozo::pg::conn::pointer;
    using statistics_type = Statistics;
    using error_context_type = std::string;

    const bozo::pg::conn& safe_native_handle() const & {return safe_handle_;}
    bozo::pg::conn& safe_native_handle() & {return safe_handle_;}
 ...
 * status is different than `bozo::transaction_status::idle` then it will not return to
...
public:
     ...
     */
    const oid_map_type& oid_map() const noexcept { return bozo::unwrap(rep_).oid_map();}

    template <typename Key, typename Value>
    void update_statistics(const Key& key, Value&& v) noexcept {
        bozo::unwrap(rep_).update_statistics(key, std::forward<Value(v)>);
    }
    const statistics_type& statistics() const noexcept { return bozo::unwrap(rep_).statistics();}
     ...
     */
    const error_context_type& get_error_context() const noexcept {
        return bozo::unwrap(rep_).get_error_context();
    }
     ...
     */
    void set_error_context(error_context_type v = {}) {
        bozo::unwrap(rep_).set_error_context(std::move(v));
    }
...
public:
    using connection_rep_type = bozo::connection_rep<typename bozo::unwrap_type<bozo::connection_type<Source>>::oid_map_type>;
     ...
     * Thread safe by default (`bozo::thread_safety<true>`).
     */
 ...
 * Thread safe by default (`bozo::thread_safety<true>`).
 *
 * @return `bozo::connection_pool` object.
 * @ingroup group-connection-functions
 * @relates bozo::connection_pool
 */
...
auto make_connection_pool(ConnectionSource&& source, const connection_pool_config& config,
    ...
    static_assert(bozo::ConnectionSource<ConnectionSource>, "source should model ConnectionSource concept");
...
#include <bozo/impl/connection_pool.h>
