...
public:
    ...
    using connection_type = bozo::connection_type<target_type>;
    ...
    void async_get_connection(TimeConstraint t, Handler&& handler) const& {
        ...
        bozo::async_get_connection(target(), timeout(t), std::forward<Handler>(handler));
    }
    ...
    void async_get_connection(TimeConstraint t, Handler&& handler) && {
        static_assert(bozo::TimeConstraint<TimeConstraint>, "should model TimeConstraint concept");
        bozo::async_get_connection(std::move(target()), timeout(t), std::forward<Handler>(handler));
    }
...
public:
    static_assert(bozo::ConnectionSource<ConnectionSource>, "ConnectionSource should model ConnectionSource concept");
