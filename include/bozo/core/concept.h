...
#error "BOZO needs BOOST_HANA_CONFIG_ENABLE_STRING_UDL to be defined"
#endif
 ...
 * * `bozo::none` --- operation is not restricted in time.
 *
 ...
 * @code
template <typename ConnectionProvider>
std::future<bozo::connection_type<ConnectionProvider>> async_io(
 ...
 * @code
template <typename ConnectionProvider>
bozo::connection_type<ConnectionProvider> async_io(
...
void Handler(bozo::error_code ec, Connection&& connection) {
    //...
}
 *@endcode
 *
...
struct Handler {
    template <typename Connection>
    void operator() (bozo::error_code ec, Connection&& connection) {
        //...
    }
};
 *@endcode
 ...
 * In case of lambda:
 *@code
auto Handler = [&] (bozo::error_code ec, auto connection) {
    //...
};
 *@endcode
 *
 * `Handler` has to handle an `bozo::error_code` object as first argument, and the `Connection` implementation
 ...
 * can be obtained with `bozo::connection_type`.
 *
 ...
 *   * `Connection` is in **null-state** --- `bozo::is_null_recursive()` returns `true`, object is useless;
 *   * `Connection` is in **bad state** --- `bozo::is_null_recursive()` returns `false`,
 *                   `bozo::connection_bad()` returns true or
 *                   `bozo::get_transaction_status()` returns not `bozo::transaction_status::idle`,
 *                    object may not be used for further operations but it may provide additional
 *                    error context via `bozo::error_message()` and `bozo::get_error_context()` functions.
 *   * `Connection` is in **good state** --- `bozo::is_null_recursive()` returns `false`,
 *                   `bozo::connection_bad()` returns true and
 *                   `bozo::get_transaction_status()` returns `bozo::transaction_status::idle`,
 *                    object may be used for further operations and may provide additional error context via
 *                   `bozo::error_message()` and `bozo::get_error_context()` functions.
 * @concept{Handler}
 */
///@}
