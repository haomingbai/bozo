 ...
 *
 * If it is needed to mark type as nullable then `bozo::is_nullable` from the type should
 * be specialized as `std::true_type` type. If the type has to be allocated in special way
 * --- `bozo::allocate_nullable()` should be specialized (see @ref group-ext-std-shared_ptr as an example).
 *
 * ### Example
 *
 ...
 * by means of given allocator if applicable. It uses `bozo::allocate_nullable_impl`
