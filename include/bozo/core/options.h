 ...
 *
 * @code
constexpr bozo::option<class foo_tag> foo;
...
constexpr decltype(auto) get_option(Map&& map, bozo::option<Key> op) {
...
constexpr decltype(auto) get_option(Map&& map, bozo::option<Key> op, T&& default_) {
    static_assert(decltype(hana::is_a<hana::map_tag>(map))::value, "Map should be boost::hana::map");
...
constexpr bozo::option<class foo_tag> foo;
constexpr bozo::option<class buzz_tag> buzz;

}

using namespace std::literals;

auto options = bozo::make_options(demo::foo="Foo"s, demo::buzz=777);
 ...
 * kinds of option factories. See `bozo::options_factory`,
 * `bozo::failover::retry_strategy` as examples of usage.
 *
...
public:
    ...
    constexpr decltype(auto) set(hana::pair<bozo::option<Key>, T> v) && {
    ...
    constexpr decltype(auto) set(hana::pair<bozo::option<Key1>, T1> v1, hana::pair<bozo::option<Key2>, T2> v2, Ts ...vs) && {
        return hana::fold(
    ...
    constexpr decltype(auto) set(hana::pair<bozo::option<Key>, T> v, Ts ...vs) const & {
        auto copy = *this;
        return std::move(copy).set(std::move(v), std::move(vs)...);
    }

    template <typename Key, typename T>
    constexpr decltype(auto) set(bozo::option<Key> op, T&& value) && {
        return std::move(*this).set(hana::make_pair(op, std::forward<T>(value)));
    }

    template <typename Key, typename T>
    constexpr decltype(auto) set(bozo::option<Key> op, T&& value) const & {
     ...
     * using namespace std::literals;
     *
     * bozo::options_factory options{demo::foo="Foo"s};
     *
     ...
     */
    template <typename Key>
    constexpr auto has(bozo::option<Key> op) const {
     ...
     */
    template <typename Key>
    constexpr decltype(auto) get(bozo::option<Key> op) const {
...
class options_factory : public bozo::options_factory_base<options_factory<Options>, Options> {
    using base = bozo::options_factory_base<options_factory<Options>, Options>;
    friend class bozo::options_factory_base<options_factory<Options>, Options>;
