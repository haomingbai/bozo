...
/**
 * @brief Default implementation of `bozo::unwrap()`
 *
 ...
 * the operation implementation `bozo::unwrap_impl`.
 *
 ...
 * Convenient shortcut is `bozo::unwrap_type`.
...
static_assert(std::is_same<bozo::unwrap_type<decltype(ref_a)>, decltype(a)>);
static_assert(std::is_same<bozo::unwrap_type<decltype(opt_a)>, decltype(a)>);
 * @endcode
 *
 * @sa bozo::unwrap_type, bozo::unwrap
 */
template <typename T>
...
#else
    using type = std::decay_t<decltype(bozo::unwrap(std::declval<T>()))>;
#endif
};

/**
 * @brief Shortcut for `bozo::get_unwrapped_type`.
 * @ingroup group-core-types
 *
 * @tparam T -- type to unwrap.
 */
