 ...
 *
 * This function is same as `bozo::request()` function except it does not provide any result data.
 ...
 * @brief Executes query with no result data expected
 *
 * This function is time constrain free shortcut to `bozo::execute()` function.
 * Its call is equal to `bozo::execute(provider, query, bozo::none, token)` call.
 *
 * @note The function does not particitate in ADL since could be implemented via functional object.
 *
 * @param provider --- connection provider object
