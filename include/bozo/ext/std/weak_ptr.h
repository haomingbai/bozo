 ...
 *
 *@code
#include <bozo/ext/std/weak_ptr.h>
 *@endcode
 *
 * `std::weak_ptr<T>` is defined as #Nullable and uses the implementation of `bozo::is_null()` examing the
 * result of `std::weak_ptr::lock()` method call.
 *
 * The `bozo::unwrap()` function is implemented dereferencing the result of `std::weak_ptr::lock()`
 * method call.
 * @note `bozo::unwrap()` from `std::weak_ptr` is not safe for reference holding since no object owning.
 */
///@{
template <typename T>
