 ...
 *
 * These options can be used with `bozo::failover::retry_strategy`.
...
struct retry_options {
    ...
    constexpr static option<tries_tag> tries{}; //!< Set number of tries, see `bozo::retry_strategy::tries()` for more information.
 ...
 * current try.
 *
 * @tparam Options --- map of `bozo::options`.
...
public:
     ...
     *
     * @param options --- map of `bozo::failover::retry_options`.
     ...
     *                  see `bozo::retry_strategy::tries()` for details about calculations.
     */
    auto get_context() const {
        return hana::concat(
            hana::make_tuple(bozo::unwrap(ctx_).provider, time_constraint()),
    ...
    std::optional<basic_try> get_next_try(bozo::error_code ec, Connection&& conn) {
    ...
    auto time_constraint() const {
        return detail::get_try_time_constraint(bozo::unwrap(ctx_).time_constraint, tries_remain());
    }
 ...
 *
 * Retry strategy is a factory for `bozo::fallback::basic_try` object. This class is
 * an options' factory (see `bozo::options_factory_base`).
 *
 * @ingroup group-failover-retry
 */
template <typename Options = decltype(hana::make_map())>
class retry_strategy : public bozo::options_factory_base<retry_strategy<Options>, Options> {
    using op = retry_options;

    friend class bozo::options_factory_base<retry_strategy<Options>, Options>;
    using base = bozo::options_factory_base<retry_strategy<Options>, Options>;
...
public:
     ...
     *
     * @param options --- `boost::hana::map` of `bozo::failover::retry_options` and values.
     */
     ...
     *
     * Default implementation for `bozo::fallback::get_first_try()` failover
    ...
    bozo::request[retry*3](pool, query, .5s, out, yield);
     * @endcode
     */
    ...
    /**
     * @brief Number of maximum tries count are setted with `bozo::retry_strategy::tries()`
     *
    ...
    /**
     * @brief Sintactic sugar for `bozo::retry_strategy::tries()`
     *
     * @param n --- number of tries
     * @return `retry_strategy` specialization object
     */
 ...
 * @return `bozo::failover::retry_strategy` specialization.
 *
 * ###Example
 *
 * Retry on a network problem and operation timed-out no more than 3 tries.
 * Each try has own time constraint calculated from total operation time constraint.
 * See `bozo::failover::retry_strategy::tries()` for time constraint details.
 *
 * @code
auto retry = failover::retry(errc::connection_error, errc::timeout)*3;
bozo::request[retry](pool, query, .5s, out, yield);
 * @endcode
 *
 * @sa `bozo::failover::retry_strategy`
 * @ingroup group-failover-retry
 */
template <typename ...ErrorConditions>
