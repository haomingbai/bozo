 ...
 *
 * The base entities are Roles which are represented by `bozo::failover::role` template
 * specializations. Every role can recover several error conditions which should be defined via
 * `bozo::failover::can_recover` customization. So if a new recover condition set is needed - a new
 ...
 *
 * There is a strategy which uses the roles. It is implemented via `bozo::failover::role_based_strategy`
 * class and can be instantiated via `bozo::failover::role_based` function.
 *
 * The strategy may work only with a special type of `ConnectionProvider` which allows being
 * bound to a specific role. Such provider is modelled by `bozo::failover::role_based_connection_provider`.
 * It has an additional method `bozo::failover::role_based_connection_provider::rebind_role`
 ...
 *
 * These options can be used with `bozo::failover::role_based_strategy`.
 ...
 * - `bozo::errc::connection_error`,
 * - `bozo::errc::type_mismatch`,
 * - `bozo::errc::protocol_error`,
 * - `bozo::errc::database_readonly`
 *
 * @note In most cases, the user should define custom roles to specify conditions to recover.
 *
 * @sa `bozo::failover::can_recover()`
 * @ingroup group-failover-role_based
 */
 ...
 * ###Customization Point
 *
 * This function should be customized for a new role via `bozo::failover::can_recover_impl` template
 * specialization.
 *
 * E.g. for `bozo::failover::replica` role the specialization may look like this:
@code
template <>
struct can_recover_impl<std::decay_t<decltype(replica)>> {
    static constexpr auto apply(const std::decay_t<decltype(replica)>&, const error_code& ec) {
...
struct master_with_custom_error_conditions {
    std::vector<bozo::errc::code> conditions;
};
...
public:
    ...
    static_assert(bozo::ConnectionSource<Source>, "Source should model a ConnectionSource concept");
    /**
     * Current source type.
    ...
    constexpr decltype(auto) rebind_role(OtherRole r) const & {
        ...
        return rebind_type{bozo::unwrap(source_).rebind_role(r), io_};
    }
    ...
    constexpr decltype(auto) rebind_role(OtherRole r) && {
        ...
        return rebind_type{bozo::unwrap(std::forward<Source>(source_)).rebind_role(r), io_};
    }
    ...
    void async_get_connection(TimeConstraint t, Handler&& h) const & {
        ...
        bozo::unwrap(source_)(io_, std::move(t), std::forward<Handler>(h));
    }
    ...
    void async_get_connection(TimeConstraint t, Handler&& h) && {
        static_assert(bozo::TimeConstraint<TimeConstraint>, "should model TimeConstraint concept");
        bozo::unwrap(std::forward<Source>(source_))(io_, std::move(t), std::forward<Handler>(h));
    }
};
 ...
 *
 * @sa `bozo::failover::make_role_based_connection_source()`
 * @ingroup group-failover-role_based
 ...
 * ###Example
 *
 * Specify different `bozo::connection_info` objects for different roles.
 *
@code
...
auto conn_info = bozo::failover::make_role_based_connection_source(
    bozo::failover::master=bozo::connection_info(cfg.master_connstr),
    bozo::failover::replica=bozo::connection_info(cfg.replica_connstr)
);
@endcode
 * @ingroup group-failover-role_based
 */
...
public:
     ...
     *
     * @param options --- map of `bozo::failover::role_based_options`.
     * @param ctx --- operation context, compartible with `bozo::failover::basic_context`
     */
    ...
    auto get_context() const {
        return hana::concat(
            hana::make_tuple(bozo::unwrap(ctx_).provider.rebind_role(role()), time_constraint()),
            bozo::unwrap(ctx_).args
        );
    }
    ...
    auto time_constraint() const {
        return detail::get_try_time_constraint(bozo::unwrap(ctx_).time_constraint, tries_left().value);
    }
     ...
     * `bozo::fallback::get_next_try()` for details.
     *
    ...
    void initiate_next_try([[maybe_unused]] bozo::error_code ec, Connection& conn, [[maybe_unused]] Initiator&& init) const {
 ...
 *
 * Role-based strategy is a factory for `bozo::fallback::role_based_try` object.
 *
 * @ingroup group-failover-role_based
 */
...
class role_based_strategy : public options_factory_base<role_based_strategy<Options>, Options> {
    ...
    friend class bozo::options_factory_base<role_based_strategy<Options>, Options>;
    using base = bozo::options_factory_base<role_based_strategy<Options>, Options>;
...
public:
     ...
     *
     * @param options --- `boost::hana::map` of `bozo::failover::role_based_options` and values.
     */
    ...
    auto get_first_try(const Operation&, const Allocator&,
        ...
        return role_based_try {
            this->options(),
            basic_context{std::move(provider), bozo::deadline(t), std::forward<Args>(args)...}
        };
    }
};
 ...
 * @return `bozo::failover::role_based_strategy` specialization.
 *
 * @note This strategy works only with `bozo::failover::role_based_connection_provider`
...
bozo::request[fallback](conn_info[io], query, .5s, out, yield);
 * @endcode
 *
 * @sa `bozo::failover::role_based_strategy`, `bozo::failover::role_based_connection_provider`
 * @ingroup group-failover-role_based
 */
