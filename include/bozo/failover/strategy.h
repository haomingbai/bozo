...
struct complex_transaction_op : base_async_operation <complex_transaction_op, Initiator> {
    ...
    decltype(auto) operator() (P&& provider, complex_transaction_data data, TimeConstraint t, CompletionToken&& token) const {
        static_assert(bozo::ConnectionProvider<P>, "provider should be a ConnectionProvider");
        static_assert(bozo::TimeConstraint<TimeConstraint>, "should model TimeConstraint concept");
        return bozo::async_initiate<CompletionToken, bozo::handler_signature<P>>(
            // Initiator and token
            get_operation_initiator(*this), token,
            // ConnectionProvider, TimeConstraint, additional arguments
            std::forward<P>(provider), bozo::deadline(t), std::move(data)
        );
    }
};
...
struct complex_transaction_impl {
    ...
    constexpr void operator()(error_code ec, Connection&& conn) {
        ...
        boost::asio::spawn(bozo::get_executor(conn), [
                data = std::move(data_),
                time_constraint = std::move(time_constraint_),
                handler = std::move(handler_)] (auto yield) mutable {
            error_code ec;
            bozo::rows_of<...> rows;
            auto conn = bozo::request(conn, time_constraint_, "..."_SQL..., bozo::into(rows), yield[ec]);
            if (!ec) {
                conn = bozo::execute(conn, time_constraint_, "..."_SQL + handle_data(rows)..., yield[ec]);
                if (!ec) {
                    return bozo::commit(conn, std::move(handler_));
                }
            }
...
struct initiate_complex_transaction {
    ...
    constexpr void operator()(Handler&& h, P&& provider, TimeConstraint t, complex_transaction_data data) const {
        bozo::begin(provider, t, complex_transaction_impl{std::move(data), t, std::forward<Handler>(h)});
    }
};
} // namespace detail
...
struct basic_context {
    static_assert(bozo::TimeConstraint<TimeConstraint>, "TimeConstraint should models bozo::TimeConstraint");
    static_assert(bozo::ConnectionProvider<ConnectionProvider>, "ConnectionProvider should models bozo::ConnectionProvider");
 ...
 * of `bozo::failover::get_first_try_impl`. E.g.:
 * @code
namespace bozo::failover {
 ...
 * of `bozo::failover::get_try_context_impl`. E.g.:
 * @code
namespace bozo::failover {
...
inline auto get_try_context(const FailoverTry& a_try) {
    auto res = detail::apply<get_try_context_impl>(bozo::unwrap(a_try));
 ...
 * `a_try.get_next_try(ec, conn)`. This behavior customisable via `bozo::failover::get_next_try_impl`
 * specialization. It will be used by default if no `bozo::failover::initiate_next_try`
 ...
 *
 * This function may be customized for a #FailoverTry via specialization
 * of `bozo::failover::get_next_try_impl`. E.g.:
 * @code
namespace bozo::failover {
...
struct get_next_try_impl<my_strategy_try> {
    ...
    static std::optional<my_strategy_try> apply(const my_strategy_try& obj, error_code ec, Conn&& conn) {
        if (ec != my_error_condition::recoverable) {
            obj.log_error("can not recover error {0} with message {1}", ec, bozo::error_message(conn));
            return std::nullopt;
        }
        obj.log_notice("recovering error {0} with message {1}", ec, bozo::error_message(conn));
        return obj;
    }
};
...
inline auto get_next_try(Try& a_try, const error_code& ec, Connection& conn) {
    return detail::apply<get_next_try_impl>(bozo::unwrap(a_try), ec, conn);
}
...
struct initiate_next_try_impl {
    ...
    static auto apply(Try& a_try, const error_code& ec, Connection& conn, Initiator init) {
        if (auto next = get_next_try(a_try, ec, conn); !bozo::is_null(next)) {
            init(std::move(next));
        }
    }
};
 ...
 * Unlike `bozo::failover::get_next_try()` it allows to use different types of
 ...
 *
 * This function may be customized for a #FailoverTry via specialization
 * of `bozo::failover::initiate_next_try_impl`. E.g. default implementation may look like this:
 * @code
namespace bozo::failover {
...
inline auto initiate_next_try(Try& a_try, const error_code& ec, Connection& conn, Initiator&& init) {
    return detail::apply<initiate_next_try_impl>(bozo::unwrap(a_try), ec, conn, std::forward<Initiator>(init));
}
...
struct continuation {
    ...
    void operator() (error_code ec, Connection&& conn) {
        static_assert(bozo::Connection<Connection>, "conn should model Connection concept");
        if (ec) {
...
inline void initiate_operation(const Operation& op, Try&& a_try, Handler&& handler) {
    hana::unpack(get_try_context(a_try), [&](auto&& ...args) {
        bozo::get_operation_initiator(op)(
