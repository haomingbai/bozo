...
#include <bozo/detail/wrap_executor.h>
...
public:
    ...
    : timer_(bozo::detail::get_operation_timer(ex, t)) {
...
private:
    using timer_type = typename bozo::detail::operation_timer<Executor>::type;
        ...
        asio::post(cancel_op{
            ...
            deadline_cancel_handler {io.get_executor(), t,
                detail::wrap_executor {
                    bozo::detail::make_strand_executor(io.get_executor()),
...
inline cancel_handle<Executor> get_cancel_handle(const Connection& connection, Executor&& executor) {
    static_assert(bozo::Connection<Connection>, "First argument should model a Connection");
...
auto cancel_op::operator() (cancel_handle<Executor>&& handle, io_context& io,
    ...
    static_assert(bozo::TimeConstraint<TimeConstraint>, "time_constraint should model TimeConstrain");
