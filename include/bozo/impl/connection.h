...
template <typename OidMap, typename Statistics>
error_code connection<OidMap, Statistics>::assign(bozo::pg::conn&& handle) {
...
template <typename OidMap, typename Statistics>
bozo::pg::conn connection<OidMap, Statistics>::release() {
    socket_.release();
    bozo::pg::conn retval;
    using std::swap;
    swap(retval, handle_);
...
inline std::string_view error_message(const Connection& conn) {
    static_assert(bozo::Connection<Connection>, "conn should model Connection");
