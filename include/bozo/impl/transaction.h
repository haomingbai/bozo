...
#include <bozo/transaction_options.h>
#include <bozo/impl/async_execute.h>
...
static_assert(!std::is_same_v<
    decltype(bozo::unwrap_connection(std::declval<bozo::transaction<bozo::connection<bozo::empty_oid_map, bozo::no_statistics>, decltype(bozo::make_options())>>())),
    decltype(std::declval<bozo::transaction<bozo::connection<bozo::empty_oid_map, bozo::no_statistics>, decltype(bozo::make_options())>>())>,
    "bozo::unwrap_connection() should return underlying connection");

static_assert(std::is_same_v<
    decltype(bozo::unwrap_connection(std::declval<bozo::transaction<bozo::connection<bozo::empty_oid_map, bozo::no_statistics>, decltype(bozo::make_options())>>())),
    decltype(std::declval<bozo::transaction<bozo::connection<bozo::empty_oid_map, bozo::no_statistics>, decltype(bozo::make_options())>>().lowest_layer())>,
    "bozo::unwrap_connection() should return underlying connection");
    ...
    void operator ()(error_code ec, Connection&& connection) {
        asio::dispatch(
            detail::bind(
                std::move(handler),
                std::move(ec),
                bozo::transaction(std::forward<Connection>(connection), std::move(options))
            )
        );
    }
};
