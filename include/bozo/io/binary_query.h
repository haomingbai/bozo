...
private:
    ...
    struct impl_type final : interface {
        static_assert(bozo::HanaSequence<Params>, "Params should be Hana.Sequence");
        static_assert(bozo::OidMap<OidMap>, "OidMap should model bozo::OidMap");
        static_assert(bozo::QueryText<Text>, "Text should model bozo::QueryText concept");
...
/**
 * @brief Concept of a type that is convertible to `bozo::binary_query`
 *
 * To be convertible to the `bozo::binary_query` type should model `Query`
 * concept or should have a valid `bozo::to_binary_query_impl<T>` overload.
 *
 * ### Customization
 *
 * A particular query object type may become `BinaryQueryConvertible` via the
 * specialization of `bozo::to_binary_query_impl<T>::apply()` template function.
...
constexpr auto oidMap = bozo::register_types<...>();

using OidMap = decltype(oidMap);

struct Query {
    virtual bozo::binary_query toBinaryQuery(const OidMap&, const std::allocator<char>&) const = 0;
    virtual ~Query() = 0;
};
 ...
 * @concept{bozo::BinaryQueryConvertible}
 */
//! @cond
template <typename T>
 ...
 * @param allocator --- allocator to use for the data of `bozo::binary_query`.
 *
 * @return `bozo::binary_query` --- the binary representation.
 *
