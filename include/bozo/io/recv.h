...
#include <bozo/detail/endian.h>
#include <bozo/detail/float.h>
#include <bozo/io/istream.h>
#include <bozo/io/type_traits.h>
 ...
 *
 * The default implementation uses `bozo::read` function to deserialize
 ...
 * equal to value returned by `bozo::size_of()` for the object --- `std::range_error`
 * will be thrown.
 *
 * To deserialize complex types like #Array or #Composite special
 * internal implementations are used.
 *
 * @note This functor requires type definition via #BOZO_PG_BIND_TYPE or
 * #BOZO_PG_DEFINE_CUSTOM_TYPE.
 *
 * ### Customization point
 *
...
struct recv_impl {
    ...
    static istream& apply(istream& in, size_type size, const OidMap&, Out& out) {
        ...
        if constexpr (DynamicSize<Out>) {
            ...
            throw bozo::system_error(error::bad_object_size,
...
inline istream& recv(istream& in, [[maybe_unused]] Oid oid, size_type size, const OidMap& oids, Out& out) {
    ...
    return detail::get_recv_impl<Out>::apply(in, size, oids, bozo::unwrap(out));
}
 ...
 *   `bozo::system_error` with `bozo::error::oid_type_mismatch` will be thrown, it uses `bozo::accepts_oid()`
 *   for this check;
 ...
 * indicates null state via `bozo::null_state_size`. `bozo::reset_nullable()` and `bozo::init_nullable()`
 * are used.
 *
 * For deserialization implementation of simple objects like strings, integers, uuid and so on
 * `bozo::recv_impl` is used. To deserialize complex types like #Array or #Composite special
 * internal implementations are used.
 *
 ...
 * See `bozo::data_frame_size()` for more details about data frame.
 *
 ...
 * object's size and object's data. See `bozo::frame_size()` for more details
 * about the frame.
 *
 * @note The function uses `bozo::recv` underhood, so all the checks will be made.
 *
 * @param in --- input stream
