 ...
 *
 * The default implementation uses `bozo::write` function to serialize
 ...
 * This function uses `bozo::send_impl` template specialization for simple types
 * like integers, floating points, strings, uuid and so on. This behaviour may be customized
 * via `bozo::send_impl` specialization.
 ...
 * This function unwraps type via `bozo::unwrap()` thus it handles properly all nullables
 * and wrapped types. If argument is #Nullable in null state the function does nothing. In
 * the other case it calls `bozo::send_impl::apply()` method for unwrapped object.
 *
 * @param out --- output stream.
...
inline ostream& send(ostream& out, const OidMap& oid_map, const In& in) {
    return bozo::is_null(in) ? out : detail::get_send_impl<In>::apply(out, oid_map, bozo::unwrap(in));
}
 ...
 * See `bozo::frame_size()` for more details about the frame.
 *
 * @param out --- output stream
 * @param oid_map --- #OidMap to determine object's oid
 * @param in --- object to send
