...
/**
 * @brief `bozo::size_of` implementation functor
 ...
 *
 * @note This template has to be defined in case of defining custom IO via `bozo::send_impl`
 * and `bozo::recv_impl`.
 *
 * ### Example
 *
 ...
 * @return `bozo::size_type` --- size of object in bytes if object is
 * not #Nullable or not in null state.
 * @return `bozo::null_state_size` --- for #Nullable object in null state.
 *
 * @note T has to have `bozo::type_traits` specialization.
 *
 * ### Customization point
 *
 * This function can be customized for the type or concept via `bozo::size_of_impl` structure
 * template specialization.
 *
 * @sa bozo::size_of_impl, BOZO_PG_BIND_TYPE, BOZO_PG_DEFINE_CUSTOM_TYPE
 */
template <typename T>
constexpr size_type size_of(const T& v) {
    ...
    return bozo::is_null(v) ? null_state_size : detail::get_size_of_impl<T>::apply(bozo::unwrap(v));
}
...
constexpr size_type data_frame_size(const T& v) {
    return sizeof(bozo::size_type) + std::max(size_of(v), 0);
}
...
constexpr size_type frame_size(const T& v) {
    return sizeof(bozo::oid_t) + data_frame_size(v);
}
