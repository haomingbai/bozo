 ...
 * `bozo::definitions::array` specialization. The library has all the necessary information
...
@endcode
 *
 * @sa BOZO_PG_DEFINE_CUSTOM_TYPE
 *
 * ### Known Types
 *
 * | Type | Size | Has Array | Short Description |
 * |------|------|-----------|-------------------|
 * | null | none | no        | BOZO pseudo-type definition for null representation |
...
#define BOZO_PG_BIND_TYPE(CppType, PgTypeName)
#else
#define BOZO_PG_BIND_TYPE(CppType, PgTypeName) \
    namespace bozo::definitions {\
    template <>\
    struct type<CppType> : bozo::pg::type_definition<decltype(PgTypeName##_s)> {\
    ...
    template <>\
    struct array<CppType> : bozo::pg::array_definition<decltype(PgTypeName##_s)>{};\
    }
#endif
...
struct type_definition {
    ...
    "via bozo::definitions::type specialization.");
};
