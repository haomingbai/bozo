 ...
 * invokes `bozo::to_const_char_impl::apply()` and should be customized via
 * `bozo::to_const_char_impl` specialization. Built-in support for types:
 * * `const char*`,
 * * `std::string`,
 * * `std::string_view`,
 * * `boost::hana::string`.
 ...
 * @endcode
 *
 * @note `bozo::to_const_char_impl::apply()` should be `noexcept`.
 *
...
struct is_query_text<T, std::void_t<
    decltype(bozo::to_const_char(std::declval<const T&>()))
>> : std::true_type {};
 ...
 * a `const char*` via `bozo::to_const_char()`. The conversion should not throw an exception.
 * It means that `query_text` models `QueryText` concept if this code is valid:
 *
 * @code
 const char* text = bozo::to_const_char(query_text);
 * @endcode
 *
 * To adapt a custom text representation as `QueryText` it is needed to customize
 * `bozo::to_const_char()` function via it's implementation. For more details see
 * the function documentation.
 *
 * @sa bozo::to_const_char()
 * @concept{QueryText}
 ...
 * invokes `bozo::get_query_text_impl::apply()` and should be customized via
 * `bozo::get_query_text_impl` specialization.
 *
 * ### Customization point
 *
 * This is one of two points (see also `bozo::get_query_params()`) which should be
 ...
 * `bozo::get_query_params_impl::apply()` and should be customized via
 * `bozo::get_query_params_impl` specialization.
 *
 * ### Customization point
 *
 * This is one of two points (see also `bozo::get_query_text()`) which should be
 * used to support your own query type in the library. E.g.:
 *
 * @code
namespace bozo {
 ...
 *
 * @code
static_assert(bozo::QueryText<decltype(bozo::get_query_text(query))>);
static_assert(bozo::HanaSequence<decltype(bozo::get_query_params(query))>);
 * @endcode
 *
 * To adapt custom type as `Query` for the library it is needed to customize
 * `bozo::get_query_text()` and `bozo::get_query_params()` via its' implementations.
 * See both functions description for more details.
 *
 * %Query may be constructed via `bozo::make_query` function or `bozo::query_builder`.
 *
 * @concept{Query}
 * @ingroup group-query-concepts
 */
