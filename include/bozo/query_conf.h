...
template <class ... QueriesT>
bozo::detail::query_description make_query_description(const hana::tuple<QueriesT ...>& queries,
                                                      const parsed_query& parsed) {
    boost::optional<bozo::detail::query_description> opt_query_description;
...
template <class ... QueriesT>
std::vector<bozo::detail::query_description> make_query_descriptions(const hana::tuple<QueriesT ...>& queries,
        const std::vector<parsed_query>& parsed) {
    std::vector<bozo::detail::query_description> result;
...
struct query_conf {
    std::vector<bozo::detail::query_description> descriptions;
    std::unordered_map<std::string_view, std::string_view> queries;

    query_conf(std::vector<bozo::detail::query_description> descriptions)
            : descriptions(std::move(descriptions)) {}
};

inline std::shared_ptr<query_conf> make_query_conf(std::vector<bozo::detail::query_description> descriptions) {
...
public:
    ...
    auto make_query() const {
        return bozo::make_query(get_description<QueryT>());
    }
    ...
    auto make_query(const typename typed_query_traits<QueryT>::parameters_type& parameters) const {
        ...
        if constexpr (detail::HasMembers<typename typed_query_traits<QueryT>::parameters_type>) {
            return hana::unpack(
                ...
                [&] (const auto& ... parameters) { return bozo::make_query(description, parameters ...); }
            );
        } else {
            return std::apply(
                [&] (const auto& ... parameters) { return bozo::make_query(description, parameters ...); },
    ...
    auto make_query(typename typed_query_traits<QueryT>::parameters_type&& parameters) const {
        ...
        if constexpr (detail::HasMembers<typename typed_query_traits<QueryT>::parameters_type>) {
            return hana::unpack(
                ...
                [&] (auto&& ... parameters) { return bozo::make_query(description, std::move(parameters) ...); }
            );
        } else {
            return std::apply(
                [&] (auto&& ... parameters) { return bozo::make_query(description, std::move(parameters) ...); },
