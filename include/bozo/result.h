...
public:
     ...
     *
     * Random access iterator. Provides access to a `bozo::value` object.
     * Since `bozo::basic_result` provides read-only access to a database
     ...
     * @return `bozo::value` --- proxy object on the value.
     */
     ...
     * @return `bozo::value` --- proxy object to a value.
     */
    value at(int index) const {
        if (index < 0 || static_cast<std::size_t>(index) >= size()) {
            throw std::out_of_range("bozo::row::at() field index "
    ...
    value at(const char* name) const {
        auto i = find(name);
        if (i == end()) {
            throw std::out_of_range(std::string("bozo::row::at() no such field name ")
                + name);
        }
        return *i;
    }
 ...
 * @tparam T --- underlying native result handler type, in common case `bozo::pg::result`.
 *
 * @ingroup group-requests-types
 */
template <typename T>
...
public:
    ...
    using row = bozo::row<std::decay_t<decltype(*std::declval<handle_type>())>>;
     ...
     *
     * Random access iterator. Provides access to a `bozo::row` object.
     * Since `bozo::basic_result` provides read-only access to a database
     * request result, all the iterators provide a read-only access to
     * a `bozo::row` object.
     */
     ...
     * @return `bozo::row` --- proxy object to a row.
     */
    ...
    row at(int i) const {
        if (i < 0 || static_cast<std::size_t>(i) >= size()) {
            throw std::out_of_range("bozo::result::at() index " + std::to_string(i) + " out of range");
        }
        return (*this)[i];
    }
 ...
 * @brief Database raw result representation
 *
 * Copyable version of `bozo::result`. The result object is useful then it needs to get an access
 * to raw data representation or the underlying `libpq` handle.
 *
 * Get this type of result may be obtained from `bozo::result`:
 *
 * @code
bozo::result res;
//...
bozo::share_result shared_res(std::move(res));
 * @endcode
 *
 * @ingroup group-requests-types
 */
...
auto make_result(T&& handle) {
    return bozo::basic_result<std::decay_t<T>>(std::forward<T>(handle));
}
