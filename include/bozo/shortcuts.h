...
//                ----         ======
//                 V             V
bozo::rows_of<std::int64_t, std::string> rows;
//           ------------  ===========

bozo::request(conn_info[io], query, bozo::into(rows), boost::asio::use_future);
@endcode
...
//                ----          ======
//                 V              V
bozo::lrows_of<std::int64_t, std::string> rows;
//            ------------  ===========
...
/**
 * @ingroup group-requests-functions
 * @brief Shortcut for create reference wrapper for `bozo::basic_result`.
 *
 * This shortcut creates reference wrapper for `bozo::basic_result` to obtain raw result into it.
 *
 * ### Example
 *
@code{cpp}
...
bozo::request(conn_info[io], query, bozo::into(res), boost::asio::use_future);
@endcode
 * @param v --- `bozo::basic_result` object for rows.
 */
template <typename T>
