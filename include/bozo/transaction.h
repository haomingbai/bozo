 ...
 * @tparam Options --- the transaction options type, see `bozo::transaction_options` for the details.
 *
...
class transaction {
public:
    static_assert(bozo::Connection<Connection>, "Connection should model Connection concept");
     ...
     *
     * If underlying `Connection` model is `Nullable` then `bozo::is_null()` should return
     ...
     *       use `bozo::begin()` instead to open a transaction with required options.
     *
    ...
    /**
     * Get the transaction options. See `bozo::transaction_options` for the details.
     *
     * @note The object should be initialized for this call.
     */
     ...
     * @note The object should be initialized for this call.
     */
    lowest_layer_type& lowest_layer() noexcept { return bozo::unwrap(impl_);}
     ...
     * @note The object should be initialized for this call.
     */
    const lowest_layer_type& lowest_layer() const noexcept {return bozo::unwrap(impl_);}

private:
    bool is_null() const noexcept { return bozo::is_null(impl_); }
...
template <typename ...Ts>
using transaction = bozo::transaction<Ts...>;
} // namespace impl
 ...
 *
 * @code
bozo::begin.with_transaction_options(bozo::make_options(Options...));
 * @endcode
 *
 * there `%Options` are available items of `bozo::transaction_options`.
 *
 * @par Example
 *
 ...
 * @brief Start new transaction
 *
 * This function is time constrain free shortcut to `bozo::begin()` function.
 * Its call is equal to `bozo::begin(provider, bozo::none, out, token)` call.
 *
 * @note The function does not particitate in ADL since could be implemented via functional object.
 *
 * @param provider --- #ConnectionProvider to get connection from.
 ...
 * @brief Commits a transaction
 *
 * This function is time constrain free shortcut to `bozo::commit()` function.
 * Its call is equal to `bozo::commit(std::move(transaction), bozo::none, token)` call.
 *
 * @note The function does not particitate in ADL since could be implemented via functional object.
 *
 * @param transaction --- open transaction to commit.
...
struct commit_op {
    ...
    auto operator() (transaction<T, Options>&& transaction, TimeConstraint t, CompletionToken&& token) const {
        static_assert(bozo::TimeConstraint<TimeConstraint>, "should model TimeConstraint concept");
        using namespace bozo::literals;
        return async_initiate<CompletionToken, handler_signature<typename bozo::transaction<T, Options>::handle_type>>(
 ...
 * Committing a transaction:
@code
conn = bozo::rollback(std::move(transaction), yield[ec]);
@endcode
 * @ingroup group-transaction-functions
 */
template <typename T, typename Options, typename TimeConstraint, typename CompletionToken>
 ...
 * @brief Commits a transaction
 *
 * This function is time constrain free shortcut to `bozo::rollback()` function.
 * Its call is equal to `bozo::rollback(std::move(transaction), bozo::none, token)` call.
 *
 * @note The function does not particitate in ADL since could be implemented via functional object.
 *
 * @param transaction --- open transaction to rollback.
 ...
 *
 * The isolation level can either be an instance of one of the child types of bozo::isolation_level or
 * `bozo::none` if no level was explicitly set.
 *
 * @param transaction transaction to get the isolation level from
 * @return sub type of bozo::isolation_level --- if the transaction isolation level has been set
 * @return bozo::none --- if no isolation level has been set
 */
template<typename... Ts>
 ...
 *
 * The mode can either be an instance of one of the child types of bozo::transaction_mode or
 * bozo::none if mode was set explicitly set.
 *
 * @param transaction transaction to get the transaction mode from
 * @return sub type of bozo::transaction_mode --- if the transaction mode has been set
 * @return bozo::none --- if no transaction mode has been specified
 */
template<typename... Ts>
 ...
 *
 * Deferrability is usually indicated using an instance of bozo::deferrable_mode, but can also
 * be any other type with a compile-time static ::value convertible to bool.
 * `bozo::none` indicates no explicit deferrability was set.
 *
 * @param transaction transaction to get the deferrability from
 * @return bozo::deferrable_mode (or any bool-convertible std::integral_constant) --- if the deferrability has been set
 * @return bozo::none --- if no deferrability has been set
 */
template<typename... Ts>
