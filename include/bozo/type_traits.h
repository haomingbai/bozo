 ...
 * For the type or template which supposed to represent an array `bozo::is_array<>`
 * should be specialized as `std::true_type`.
 *
 * @par Requirements
 *
 * `#include <bozo/io/array.h>` should be included for arrays support.
 *
 * @par Concrete models
 *
 ...
 * is a fixed-size container, so it should be pointed via `bozo::fit_array_size_impl`
 * specialization.
 *
 * @code
namespace bozo {
...
struct type_traits {
    ...
    using oid = <implementation defined>; //!< `bozo::oid_constant` with Oid of the built-in type or null_oid_t for non built-in type
    using size = <implementation defined>; //!< `bozo::size_constant` with size of the type object in bytes or `bozo::dynamic_size` in other case
};
#endif
 ...
 * `bozo::is_composite<>` is specialized for this type as `std::true_type`.
 *
 * @par Requirements
 *
 * `#include <bozo/io/composite.h>` should be included for the composite support.
 *
 * @par Concrete models
 *
...
#define BOZO_PG_DEFINE_TYPE_(Type, Name, OidType, Size) \
...
#define BOZO_PG_DEFINE_TYPE(Type, Name, Oid, Size) \
    BOZO_PG_DEFINE_TYPE_(Type, Name, oid_constant<Oid>, Size)

#define BOZO_PG_DEFINE_TYPE_ARRAY_(Type, OidType) \
    namespace bozo::definitions {\
    template <>\
    struct array<Type> : detail::array_definition<Type, OidType>{};\
    }

#define BOZO_PG_DEFINE_TYPE_AND_ARRAY_(Type, Name, OidType, ArrayOidType, Size) \
    BOZO_PG_DEFINE_TYPE_(Type, Name, OidType, Size) \
    BOZO_PG_DEFINE_TYPE_ARRAY_(Type, ArrayOidType)

/**
 * @brief [[DEPRECATED]] Helper macro to define type mapping
 *
 * In general type mapping is provided via `bozo::definitions::type` and
 * `bozo::definitions::array` specialization.
 * To reduce the boilerplate code the macro exists.
 *
 * @note This macro is deprecated, use #BOZO_PG_BIND_TYPE instead.
 ...
 */
#ifdef BOZO_DOCUMENTATION
#define BOZO_PG_DEFINE_TYPE_AND_ARRAY(Type, Name, Oid, ArrayOid, Size)
#else
#define BOZO_PG_DEFINE_TYPE_AND_ARRAY(Type, Name, Oid, ArrayOid, Size) \
    BOZO_PG_DEFINE_TYPE_AND_ARRAY_(Type, Name, oid_constant<Oid>, oid_constant<ArrayOid>, Size)
#endif
...
BOZO_PG_DEFINE_CUSTOM_TYPE(smtp::message, "code.message")
 * @endcode
 * @sa BOZO_PG_BIND_TYPE
 * @ingroup group-type_system-mapping
 */
#ifdef BOZO_DOCUMENTATION
#define BOZO_PG_DEFINE_CUSTOM_TYPE(Type, Name [, Size])
#else
#define BOZO_PG_DEFINE_CUSTOM_TYPE_IMPL_3(Type, Name, Size) \
    BOZO_PG_DEFINE_TYPE_AND_ARRAY_(Type, Name, void, void, Size)

#define BOZO_PG_DEFINE_CUSTOM_TYPE_IMPL_2(Type, Name) \
    BOZO_PG_DEFINE_CUSTOM_TYPE_IMPL_3(Type, Name, dynamic_size)

#define BOZO_PG_DEFINE_CUSTOM_TYPE(...)\
    BOOST_PP_OVERLOAD(BOZO_PG_DEFINE_CUSTOM_TYPE_IMPL_,__VA_ARGS__)(__VA_ARGS__)
#endif
...
BOZO_PG_DEFINE_CUSTOM_TYPE(custom_type, "code.custom_type")

//...
// Creating ConnectionSource for futher requests to a database
const bozo::connection_info conn_source("...", register_types<custom_type>());
 * @endcode
 *
...
constexpr auto type_oid(const OidMap&) noexcept
        -> Require<BuiltIn<T>, oid_t> {
    static_assert(bozo::OidMap<OidMap>, "map should model OidMap concept");
 ...
 *
 * @code
static_assert(empty(bozo::empty_oid_map{}));
 * @endcode
