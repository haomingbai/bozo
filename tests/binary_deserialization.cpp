...
struct recv : Test {
    bozo::empty_oid_map oid_map{};
    StrictMock<pg_result_mock> mock{};
    bozo::value<pg_result_mock> value{{&mock, 0, 0}};
};
...
TEST_F(recv, should_throw_system_error_if_oid_does_not_match_the_type) {
    ...
    int x;
    EXPECT_THROW(bozo::recv(value, oid_map, x), bozo::system_error);
}
...
TEST_F(recv, should_throw_for_a_null_value_if_receiving_type_is_not_nullable) {
    ...
    std::string got;
    EXPECT_THROW(bozo::recv(value, oid_map, got), std::invalid_argument);
}
...
struct recv_row : Test {
    bozo::empty_oid_map oid_map{};
    StrictMock<pg_result_mock> mock{};
    bozo::row<pg_result_mock> row{{&mock, 0, 0}};
};
...
TEST_F(recv_row, should_throw_range_error_if_size_of_tuple_does_not_equal_to_row_size) {
    ...
    EXPECT_THROW(bozo::recv_row(row, oid_map, out), std::range_error);
}
...
struct recv_result : Test {
    bozo::empty_oid_map oid_map{};
    StrictMock<pg_result_mock> mock{};
    bozo::basic_result<pg_result_mock*> res{&mock};
};
...
TEST_F(recv_result, send_convert_INT4OID_and_TEXTOID_to_fusion_adapted_structures_vector_via_back_inserter) {
    ...
    bozo::recv_result(res, oid_map, std::back_inserter(got));
...
TEST_F(recv_result, send_convert_INT4OID_and_TEXTOID_to_fusion_adapted_structures_vector_via_iterator) {
    ...
    std::vector<fusion_adapted_test_result> got;
    got.resize(2);
    bozo::recv_result(res, oid_map, got.begin());
    EXPECT_EQ(got[0].digit, 7);
    EXPECT_EQ(got[0].text, "test");
    EXPECT_EQ(got[1].digit, 7);
    EXPECT_EQ(got[1].text, "test");
...
TEST_F(recv_result, send_returns_result_then_result_requested) {
    bozo::basic_result<pg_result_mock*> got;
    bozo::recv_result(res, oid_map, got);
...
TEST_F(recv, should_convert_UUIDOID_to_uuid) {
    ...
    bozo::recv(value, oid_map, result);
...
struct to_duration : TestWithParam<std::tuple<bozo::detail::pg_interval, std::chrono::microseconds>> {
};

TEST_P(to_duration, should_convert_pg_interval_to_chrono_microseconds) {
    const auto [interval, expected] = GetParam();

    const auto result = bozo::detail::to_chrono_duration(interval);
    EXPECT_EQ(result, expected);
}

INSTANTIATE_TEST_SUITE_P(convert_success, to_duration, Values(
    std::make_tuple(bozo::detail::pg_interval{       36672013014LL,         9,          92}, 239278272013014us),
    std::make_tuple(bozo::detail::pg_interval{      -49727986986LL,       -20,          93}, 239278272013014us),
    std::make_tuple(bozo::detail::pg_interval{   239278272013014LL,         0,           0}, 239278272013014us),

    std::make_tuple(bozo::detail::pg_interval{  3333333333333333LL,          0,          0},  3333333333333333us),
    std::make_tuple(bozo::detail::pg_interval{                 0LL,     200000,          0}, 17280000000000000us),
    std::make_tuple(bozo::detail::pg_interval{                 0LL,          0,       5555}, 14398560000000000us),

    std::make_tuple(bozo::detail::pg_interval{      -14454775808LL,  -106751991,         0}, std::chrono::microseconds::min()),
    std::make_tuple(bozo::detail::pg_interval{        71945224192LL, -106751992,         0}, std::chrono::microseconds::min()),
    std::make_tuple(bozo::detail::pg_interval{      -532854775808LL, -555555555,  14960119}, std::chrono::microseconds::min()),

    std::make_tuple(bozo::detail::pg_interval{        14454775807LL,  106751991,         0}, std::chrono::microseconds::max()),
    std::make_tuple(bozo::detail::pg_interval{       -71945224193LL,  106751992,         0}, std::chrono::microseconds::max()),
    std::make_tuple(bozo::detail::pg_interval{9223370740854775807LL,  555555555, -18518518}, std::chrono::microseconds::max())
));

INSTANTIATE_TEST_SUITE_P(convert_success_with_overflow, to_duration, Values(
    std::make_tuple(bozo::detail::pg_interval{-14454775809LL, -106751991, 0}, std::chrono::microseconds::min()),
    std::make_tuple(bozo::detail::pg_interval{ 14454775808LL,  106751991, 0}, std::chrono::microseconds::max()),

    std::make_tuple(
        bozo::detail::pg_interval{
            std::numeric_limits<decltype(bozo::detail::pg_interval::microseconds)>::min(),
            std::numeric_limits<decltype(bozo::detail::pg_interval::days)>::min(),
            std::numeric_limits<decltype(bozo::detail::pg_interval::months)>::min()
        },
    ...
    std::make_tuple(
        bozo::detail::pg_interval{
            std::numeric_limits<decltype(bozo::detail::pg_interval::microseconds)>::max(),
            std::numeric_limits<decltype(bozo::detail::pg_interval::days)>::max(),
            std::numeric_limits<decltype(bozo::detail::pg_interval::months)>::max()
        },
