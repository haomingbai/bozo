#include <bozo/detail/deadline.h>
#include "../test_asio.h"
...
struct stream_mock {
    using executor_type = bozo::tests::io_context::executor_type;
    MOCK_METHOD0(cancel, void());
    MOCK_METHOD0(get_executor, executor_type());
};

struct io_deadline_handler : Test {
    bozo::tests::steady_timer_mock timer;
    bozo::tests::execution_context io;
    StrictMock<bozo::tests::callback_gmock<int>> continuation;
    StrictMock<bozo::tests::executor_mock> continuation_executor;
    ...
    io_deadline_handler() {
        ...
        EXPECT_CALL(continuation, get_executor())
            .WillRepeatedly(Return(bozo::tests::executor{continuation_executor, io}));
        EXPECT_CALL(timer, async_wait(_)).WillOnce(SaveArg<0>(&on_timer_expired));
    }

    using deadline_handler = bozo::detail::io_deadline_handler<stream_mock, std::decay_t<decltype(bozo::tests::wrap(continuation))>, int>;
};
...
TEST_F(io_deadline_handler, should_cancel_timer_and_call_handler_with_error_and_result_on_normal_call) {
    ...
    EXPECT_CALL(continuation_executor, post(_)).WillOnce(InvokeArgument<0>());
    EXPECT_CALL(continuation, call(Eq(bozo::tests::error::error), 777));
    using bozo::tests::wrap;
    deadline_handler handler{stream, time_point{}, wrap(continuation)};
    handler(bozo::tests::error::error, 777);
