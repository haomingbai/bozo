...
TEST(IsApplicable, should_return_true_for_applicable_functional_arguments) {
    const bool res = bozo::detail::IsApplicable<test_functional, std::string, int>;
    EXPECT_TRUE(res);
}

TEST(IsApplicable, should_return_false_for_non_applicable_functional_arguments) {
    const bool res = bozo::detail::IsApplicable<test_functional, std::string, std::string>;
    EXPECT_FALSE(res);
}

TEST(result_of, should_return_type_of_functional_result) {
    using type = bozo::detail::result_of<test_functional, std::string, int>;
    const bool res = std::is_same_v<type, int>;
    EXPECT_TRUE(res);
}

TEST(apply, should_invoke_functional_and_return_result) {
    const auto res = bozo::detail::apply<test_functional>(std::string{}, 42);
    EXPECT_EQ(res, 42);
}

TEST(apply, should_dispatch_functional_by_first_argument) {
    const auto res = bozo::detail::apply<test_dispatch>(std::string{}, 42);
    EXPECT_EQ(res, 777);
}

TEST(apply, should_be_no_noexcept_if_implementation_is_not_noexcept) {
    EXPECT_FALSE(noexcept(bozo::detail::apply<test_functional>(std::string{}, 42)));
}

TEST(apply, should_be_noexcept_if_implementation_is_noexcept) {
    EXPECT_TRUE(noexcept(bozo::detail::apply<test_noexcept_functional>(std::string{}, 42)));
}
