...
TEST(query_builder_text, with_std_string_text_returns_string_text) {
    EXPECT_EQ(bozo::make_query_builder(hana::make_tuple(bozo::make_query_text(std::string("SELECT 1")))).text(), "SELECT 1");
}

TEST(query_builder_text, with_std_string_text_and_params_returns_string_text_with_with_placeholders_for_each_param) {
    EXPECT_EQ((bozo::make_query_text(std::string("SELECT ")) + std::int32_t(42)
                + bozo::make_query_text(std::string(" + ")) + std::int32_t(42)).text(), "SELECT $1 + $2");
}
...
BOZO_PG_DEFINE_CUSTOM_TYPE(bozo::tests::some_type, "some_type")
