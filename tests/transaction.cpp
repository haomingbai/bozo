...
constexpr auto custom_opt = bozo::make_options(bozo::transaction_options::isolation_level = bozo::isolation_level::serializable);
[[maybe_unused]] constexpr auto custom_begin = bozo::begin.with_transaction_options(custom_opt);
...
struct transaction : Test {
    ...
    io_context io;
    using connection_t = decltype(make_connection(connection, io, native_handle, bozo::empty_oid_map_c));
    connection_t conn = make_connection(connection, io, native_handle, bozo::empty_oid_map_c);
    using options_t = decltype(bozo::make_options());
    options_t options = bozo::make_options();

    static_assert(
        std::is_default_constructible_v<bozo::transaction<connection_t, options_t>>,
        "transaction should be default constructible for default-constructible connection"
    );

    static_assert(
        !std::is_default_constructible_v<bozo::transaction<typename connection_t::element_type, options_t>>,
...
TEST_F(transaction, lowest_layer__should_return_reference_on_unwrapped_connection) {
    bozo::transaction<connection_t, options_t> t(connection_t{conn}, options);
...
TEST_F(transaction, should_be_in_null_state_for_default_constructible_connection_in_null_state) {
    bozo::transaction<connection_t, options_t> t;
    EXPECT_TRUE(bozo::is_null(t));
}
...
TEST_F(transaction, should_not_be_in_null_state_for_connection_not_in_null_state) {
    bozo::transaction<connection_t, options_t> t(std::move(conn), options);
    EXPECT_FALSE(bozo::is_null(t));
}
