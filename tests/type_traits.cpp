...
TEST(is_null, should_return_true_for_non_initialized_optional) {
    EXPECT_TRUE(bozo::is_null(boost::optional<int>{}));
}

TEST(is_null, should_return_false_for_initialized_optional) {
    EXPECT_TRUE(!bozo::is_null(boost::optional<int>{0}));
}

TEST(is_null, should_return_false_for_valid_std_weak_ptr) {
    auto ptr = std::make_shared<int>();
    EXPECT_TRUE(!bozo::is_null(std::weak_ptr<int>{ptr}));
}
...
TEST(is_null, should_return_false_for_valid_boost_weak_ptr)  {
    ...
    EXPECT_TRUE(!bozo::is_null(boost::weak_ptr<int>(ptr)));
}
...
TEST(is_null_recursive, should_return_true_if_nested_nullable_type_is_null) {
    ...
    EXPECT_FALSE(bozo::is_null(obj));
    EXPECT_TRUE(bozo::is_null_recursive(obj));
}
...
TEST(is_null_recursive, should_return_false_if_nested_nullable_types_contains_no_null) {
    ...
    EXPECT_FALSE(bozo::is_null(obj));
    EXPECT_FALSE(bozo::is_null_recursive(obj));
}

TEST(is_null_recursive, should_return_false_for_non_nullable_type) {
    EXPECT_FALSE(bozo::is_null_recursive(int()));
}
...
TEST(unwrap_type, should_unwrap_type_with_no_adl) {
    ...
    unwrap(type{});
    const bool is_same = std::is_same_v<bozo::unwrap_type<type>, type>;
    EXPECT_TRUE(is_same);
}

TEST(unwrap, should_unwrap_type) {
    boost::optional<int> n(7);
    EXPECT_EQ(bozo::unwrap(n), 7);
}
...
TEST(unwrap, should_unwrap_std_reference_wrapper) {
    int n(7);
    EXPECT_EQ(bozo::unwrap(std::ref(n)), 7);
}

TEST(unwrap_recursive, should_unwrap_type) {
    boost::optional<int> n(7);
    EXPECT_EQ(bozo::unwrap_recursive(n), 7);
}
...
BOZO_PG_DEFINE_CUSTOM_TYPE(some_type, "some_type")
BOZO_PG_DEFINE_TYPE(builtin_type, "builtin_type", 5, bytes<8>)

BOOST_FUSION_DEFINE_STRUCT(
    (bozo)(tests), fusion_adapted,
    (std::string, name)
    (int, age))
...
TEST(type_name, should_return_type_name_object) {
    ...
    EXPECT_EQ(bozo::type_name(some_type{}), "some_type"s);
}

TEST(size_of, should_return_size_from_traits_for_static_size_type) {
    EXPECT_EQ(bozo::size_of(builtin_type{}), 8u);
}

TEST(size_of, should_return_size_from_method_size_for_dynamic_size_objects) {
    EXPECT_EQ(bozo::size_of(some_type{}), 1000u);
}

TEST(type_oid, should_return_oid_from_traits_for_buildin_type) {
    const auto oid_map = bozo::register_types<>();
    EXPECT_EQ(bozo::type_oid(oid_map, builtin_type{}), 5u);
}

TEST(type_oid, should_return_oid_from_oid_map_for_custom_type) {
    auto oid_map = bozo::register_types<some_type>();
    const auto val = some_type{};
    bozo::set_type_oid<some_type>(oid_map, 333);
    EXPECT_EQ(bozo::type_oid(oid_map, val), 333u);
}

struct accepts_oid : Test {
    decltype(bozo::register_types<some_type>()) oid_map = bozo::register_types<some_type>();
};

TEST_F(accepts_oid, should_return_true_for_type_with_oid_in_map_and_same_oid_argument) {
    const auto val = some_type{};
    bozo::set_type_oid<some_type>(oid_map, 222);
    EXPECT_TRUE(bozo::accepts_oid(oid_map, val, 222));
}

TEST_F(accepts_oid, should_return_false_for_type_with_oid_in_map_and_different_oid_argument) {
    const auto val = some_type{};
    bozo::set_type_oid<some_type>(oid_map, 222);
    EXPECT_TRUE(!bozo::accepts_oid(oid_map, val, 0));
}
